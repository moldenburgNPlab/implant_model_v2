...
def fig_2D_contour():
    ...
    if filled_contours:
        ...
        manual_locations = [(0.3, 0.6), (1.0, 0.5), (1.0, 0.2), (1.7, 0.05)]
        ...
        cs4 = axs[0, 1].contour(1 - rpos_vals, surv_vals, tripol_thr,
                                np.arange(all_min, all_max, (all_max - all_min) / n_levels),
                                extend='both', linestyles = 'solid', colors='k')
        manual_locations = [(0.3, 0.6), (0.35, 0.3), (0.9, 0.6), (0.9, 0.2), (1.7, 0.1)]

        axs[0, 1].clabel(cs4, cs4.levels, inline=True, fmt=fmt, fontsize=10, manual=manual_locations)
        axs[0, 1].set_title('Tripolar', fontsize=12)
        axs[0, 1].set_xlabel('Electrode distance (mm)')
        axs[0, 1].text(1 - high_rpos_val - 0.15, high_surv_val - 0.06, labels[0], horizontalalignment='left',
                       verticalalignment='bottom', fontweight='bold')
        axs[0, 1].text(1 - low_rpos_val - 0.15, high_surv_val - 0.06, labels[1], horizontalalignment='left',
                       verticalalignment='bottom', fontweight='bold')
        axs[0, 1].text(1 - high_rpos_val + lab_shift, low_surv_val, labels[2], horizontalalignment='left',
                       verticalalignment='bottom', fontweight='bold')
        axs[0, 1].text(1 - low_rpos_val + lab_shift, low_surv_val, labels[3], horizontalalignment='left',
    ...
    axs[1, 1].axes.set_ylim([-30, 30])

    filename = 'Fig4_2D_contour' + es_text + '.eps'
    ...
    figname = 'Fig5_contour_examples.eps'
